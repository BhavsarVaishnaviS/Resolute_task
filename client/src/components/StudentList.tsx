...
const StudentList: React.FC = () => {
    ...
    const fetchStudents = useCallback(async () => {
        ...
        try {
            const res = await api.get('/students');
            // Backend returns per-field Level-1 encrypted maps. Frontend decrypts each field.
    ...
    return (
        <div style={styles.page}>
            ...
            <div style={styles.stats}>
                ...
                <div style={{ ...styles.statCard, background: '#f0fdf4' }}>
                    <span style={{ ...styles.statNum, color: '#166534' }}>🔐 Per-Field</span>
                    <span style={styles.statLabel}>2-Layer AES Encryption</span>
                </div>
            </div>
            ...
            {showForm && (
                <StudentForm
                    ...
                    onSuccess={() => {
                        setShowForm(false);
                        fetchStudents();
                        showToast(editTarget ? 'Student updated!' : 'Student registered!');
                    }}
                    onCancel={() => setShowForm(false)}
                />
            )}
...
const styles: Record<string, React.CSSProperties> = {
    ...
    toolbar: { padding: '24px 32px', display: 'flex', gap: '12px', alignItems: 'center' },
    searchInput: {
        flex: 1, padding: '12px 16px', border: '1.5px solid #e5e7eb',
        borderRadius: '8px', fontSize: '14px', outline: 'none',
    },
    ...
    stats: { display: 'flex', gap: '16px', padding: '0 32px 24px' },
    statCard: {
        background: '#fff', borderRadius: '12px', padding: '16px 24px',
        boxShadow: '0 2px 8px rgba(0,0,0,0.06)', display: 'flex', flexDirection: 'column', gap: '4px',
    },
    ...
    emptyState: { textAlign: 'center', padding: '80px', color: '#9ca3af', fontSize: '16px' },
    overlay: {
        position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)',
        display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000,
    },
